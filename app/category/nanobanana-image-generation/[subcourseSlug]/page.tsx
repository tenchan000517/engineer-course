...
export async function generateStaticParams() {
  return [
    { subcourseSlug: 'nanobanana-beginner' },
  ];
}
...
export default async function SubcoursePage({
  ...
  // 親カテゴリを取得
  const parentCategory = getCategoryBySlug('nanobanana-image-generation');
  ...
  const colors = colorMap[parentCategory.color] || colorMap.amber;
    ...
    <div className="min-h-screen bg-gray-50 overflow-x-hidden">
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-8 md:py-16 w-full">
        ...
        <div className="flex items-center gap-2 text-gray-600 mb-8 text-sm">
          ...
          <Link href="/category/nanobanana-image-generation" className="hover:text-gray-900">
        ...
        <div className="text-center mb-12 md:mb-16">
          <div
            className={`inline-flex items-center justify-center w-16 h-16 ${colors.bg} text-white rounded-2xl mb-4`}
          >
            {iconMap.image}
          </div>
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-12">
          <span className={`px-3 py-1 ${colors.tagBg} ${colors.tagText} rounded-full text-sm`}>
            Gemini API
          </span>
        ...
        <div className="mb-12 md:mb-16">
          ...
          <div className="space-y-3 md:space-y-4">
            {modules.map((module) => (
              <Link
                key={module.slug}
                href={`/category/nanobanana-image-generation/${subcourseSlug}/${module.slug}`}
              ...
              >
                <div className="p-4 md:p-6 flex items-center gap-3 md:gap-4">
                  <div
                    className={`${colors.bg} w-10 h-10 md:w-12 md:h-12 rounded-lg flex items-center justify-center text-white font-bold text-lg md:text-xl flex-shrink-0`}
                  >
                    {module.order}
                  </div>
        ...
        {modules.length > 0 && (
          <div className="text-center pb-8">
            <Link
              href={`/category/nanobanana-image-generation/${subcourseSlug}/${modules[0].slug}`}
              className={`inline-block ${colors.bg} hover:opacity-90 text-white font-bold text-base md:text-lg px-6 md:px-8 py-3 md:py-4 rounded-lg transition-opacity`}
            >
              講座をはじめる
            </Link>
          </div>
