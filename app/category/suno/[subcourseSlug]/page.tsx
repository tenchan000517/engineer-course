...
const subcourseDirectoryMap: Record<string, string> = {
  'suno-beginner': 'suno-beginner',
};

export async function generateStaticParams() {
  return [
    { subcourseSlug: 'suno-beginner' },
  ];
}
...
const colorMap: Record<string, { bg: string; text?: string; border: string }> = {
  orange: { bg: 'bg-orange-500', border: '#f97316' },
};
...
export default async function SubcoursePage({
  ...
  // 親カテゴリ（suno）を取得
  const parentCategory = getCategoryBySlug('suno');
  ...
  const colors = colorMap[parentCategory.color] || colorMap.orange;
    ...
    <div className="min-h-screen bg-gray-50 overflow-x-hidden">
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-8 md:py-16 w-full">
        ...
        <div className="flex items-center gap-2 text-gray-600 mb-8 text-sm">
          ...
          <Link href="/category/suno" className="hover:text-gray-900">
        ...
        <div className="mb-12 md:mb-16">
          ...
          <div className="space-y-3 md:space-y-4">
            {modules.map((module) => (
              <Link
                key={module.slug}
                href={`/category/suno/${subcourseSlug}/${module.slug}`}
        ...
        {modules.length > 0 && (
          <div className="text-center pb-8">
            <Link
              href={`/category/suno/${subcourseSlug}/${modules[0].slug}`}
              className={`inline-block ${colors.bg} hover:bg-orange-600 text-white font-bold text-base md:text-lg px-6 md:px-8 py-3 md:py-4 rounded-lg transition-opacity`}
            >
              講座をはじめる
            </Link>
          </div>
