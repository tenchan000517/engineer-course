...
export default async function SubcoursePage({
  ...
  const colors = colorMap[parentCategory.color] || colorMap.white;
    ...
    <div className="min-h-screen bg-gray-50 overflow-x-hidden">
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-8 md:py-16 w-full">
        ...
        <div className="flex items-center gap-2 text-gray-600 mb-8 text-sm">
          ...
          <Link href="/category/sora2" className="hover:text-gray-900">
        ...
        <div className="mb-12 md:mb-16">
          ...
          <div className="space-y-3 md:space-y-4">
            {modules.map((module) => (
              <Link
                key={module.slug}
                href={`/category/sora2/${subcourseSlug}/${module.slug}`}
        ...
        {modules.length > 0 && (
          <div className="text-center pb-8">
            <Link
              href={`/category/sora2/${subcourseSlug}/${modules[0].slug}`}
              className={`inline-block ${colors.bg} hover:bg-gray-100 ${colors.text || 'text-white'} font-bold text-base md:text-lg px-6 md:px-8 py-3 md:py-4 rounded-lg transition-opacity`}
            >
              講座をはじめる
            </Link>
          </div>
