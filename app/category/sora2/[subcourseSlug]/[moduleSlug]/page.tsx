...
export async function generateStaticParams() {
  ...
  // sora2-basics のモジュール
  const sora2Modules = getModulesByCategory('sora2');
  sora2Modules.forEach((module) => {
    params.push({
      subcourseSlug: 'sora2-basics',
      moduleSlug: module.slug,
    });
  });

  // sora2-advanced のモジュール
  const advancedModules = getModulesByCategory('sora2-advanced');
  advancedModules.forEach((module) => {
    params.push({
      subcourseSlug: 'sora2-advanced',
      moduleSlug: module.slug,
    });
  });
...
export default async function SubcourseModulePage({
  ...
  // 親カテゴリ（sora2）を取得
  const parentCategory = getCategoryBySlug('sora2');
  ...
  const basePath = `/category/sora2/${subcourseSlug}`;
    ...
    <div className="flex min-h-screen bg-gray-50 overflow-x-hidden w-full">
      ...
      <aside className="hidden md:block fixed left-0 top-0 h-screen w-64 bg-white shadow-lg overflow-y-auto z-40">
        <div className="p-4 border-b">
          <Link href="/category/sora2" className="text-sm text-gray-500 hover:text-gray-700 flex items-center gap-1 mb-2">
          ...
          <div className="flex justify-between items-center mb-6 md:mb-8 gap-2 w-full min-w-0">
            ...
            {nextModule ? (
              <Link
                ...
                className="flex items-center gap-1 md:gap-2 bg-gray-800 hover:bg-gray-900 text-white font-medium px-3 md:px-6 py-2 md:py-3 rounded-lg transition-colors text-sm md:text-base"
              >
