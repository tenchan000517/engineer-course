...
export async function generateStaticParams() {
  ...
  // kling-basics のモジュール
  const klingModules = getModulesByCategory('kling');
  klingModules.forEach((module) => {
    params.push({
  ...
  // kling-advanced のモジュール
  const advancedModules = getModulesByCategory('kling-advanced');
...
export default async function SubcourseModulePage({
  ...
  const basePath = `/category/kling/${subcourseSlug}`;
    ...
    <div className="flex min-h-screen bg-gray-50 overflow-x-hidden w-full">
      ...
      <aside className="hidden md:block fixed left-0 top-0 h-screen w-64 bg-white shadow-lg overflow-y-auto z-40">
        <div className="p-4 border-b">
          <Link href="/category/kling" className="text-sm text-gray-500 hover:text-gray-700 flex items-center gap-1 mb-2">
          ...
          <div className="flex justify-between items-center mb-6 md:mb-8 gap-2 w-full min-w-0">
            ...
            {nextModule ? (
              <Link
                ...
                className="flex items-center gap-1 md:gap-2 bg-green-600 hover:bg-green-700 text-white font-medium px-3 md:px-6 py-2 md:py-3 rounded-lg transition-colors text-sm md:text-base"
              >
