...
const subcourseDirectoryMap: Record<string, string> = {
  'google-flow-basics': 'google-flow',
  'google-flow-advanced': 'google-flow-advanced',
};
...
export async function generateStaticParams() {
  ...
  // google-flow-basics のモジュール
  const googleFlowModules = getModulesByCategory('google-flow');
  googleFlowModules.forEach((module) => {
    params.push({
      subcourseSlug: 'google-flow-basics',
      moduleSlug: module.slug,
    });
  });

  // google-flow-advanced のモジュール
  const advancedModules = getModulesByCategory('google-flow-advanced');
  advancedModules.forEach((module) => {
    params.push({
      subcourseSlug: 'google-flow-advanced',
      moduleSlug: module.slug,
    });
  });
...
export default async function SubcourseModulePage({
  ...
  // 親カテゴリ（google-flow）を取得
  const parentCategory = getCategoryBySlug('google-flow');
  ...
  const basePath = `/category/google-flow/${subcourseSlug}`;
    ...
    <div className="flex min-h-screen bg-gray-50 overflow-x-hidden w-full">
      ...
      <aside className="hidden md:block fixed left-0 top-0 h-screen w-64 bg-white shadow-lg overflow-y-auto z-40">
        <div className="p-4 border-b">
          <Link href="/category/google-flow" className="text-sm text-gray-500 hover:text-gray-700 flex items-center gap-1 mb-2">
