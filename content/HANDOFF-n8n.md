...
**目的**: 徹底的な調査を行い、サステナブルな最適シート設計を導き出す
...
**成果物**: 上記全てに対応可能な柔軟なシート設計（posts / media テーブル構成の最終版）

**既存リソース**:
- 調査・設計書: `content/modules/n8n/module-04-research.md`（初期調査あり）
- テスト用スプレッドシート: n8n-test (ID: 1jnsUM1eNojeLmAGkZwEzLetBrB2CUjzRVoJ_3uKmKIQ)

**注意**: 安直に最終フローを決めつけず、様々な用途に対応できる柔軟な設計を目指すこと

---

### Module 06: AI連携（キャプション/画像生成）（予定変更）

**テーマ**: OpenAI/Claude APIを使ったコンテンツ生成

**状態**: Module 05完了後に実施

**学習内容（予定）**:
- OpenAI API（またはClaude API）のCredential設定
- スプレッドシートのデータを元にキャプション生成
- 画像生成API（DALL-E等）との連携

---

### Module 07: 統合ワークフロー（予定）

**テーマ**: シート→AI生成→Instagram投稿の一連の自動化

**状態**: Module 05, 06完了後に実施

**学習内容（予定）**:
- スプレッドシートから投稿予定データを取得
- AI生成でキャプション・画像を作成
- Instagram投稿を実行（単一/カルーセル/リール）
- 投稿結果をシートに記録（ステータス更新）
- スケジュール実行の設定
...
**最終更新**: 2025-12-04 （Module 04 完了、Module 05 指示追加）
