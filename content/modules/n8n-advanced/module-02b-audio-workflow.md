...
このモジュールで作成したワークフローのテンプレートをダウンロードできます：

[音声合成ワークフロー（テンプレート）](/n8n-advanced/audio-workflow-template.json)

**使用方法**:
1. ファイルをダウンロード
2. n8nで「Import from File」からインポート
3. 以下のプレースホルダーを自分の値に置換：
   - `YOUR_SPREADSHEET_ID` → Google SheetsのスプレッドシートID
   - `YOUR_SHEET_GID` → canva_AシートのgID
   - `YOUR_PARENT_FOLDER_ID` → Google Driveの親フォルダID
   - `YOUR_VOICE_ID` → Fish AudioのVoice ID
   - `YOUR_CREDENTIAL_ID` → 各Credentialを再設定
