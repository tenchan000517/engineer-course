...
VS Codeが開き、n8nフォルダが表示されます。

> **「code」コマンドが使えない場合**: VS Codeを開いて `Ctrl + Shift + P` →「Shell Command: Install 'code' command in PATH」を実行してください。

##### 手順3: VS Code内でターミナルを開く

VS Codeで `Ctrl + `` （バッククォート）を押すと、下部にターミナルが開きます。
...
- [ ] n8nディレクトリが作成された
- [ ] VS Codeでn8nフォルダを開いている（または、ターミナルでn8nディレクトリに移動している）
...
以下のリンクからdocker-compose.ymlをダウンロードして、`C:/n8n/` フォルダに保存してください：

📥 [docker-compose.yml をダウンロード](/n8n-setup/docker-compose.yml)

#### 方法B: VS Codeで手動作成

1. VS Codeの左側「エクスプローラー」で **n8nフォルダ** を右クリック
2. 「**新しいファイル**」をクリック
3. ファイル名に `docker-compose.yml` と入力してEnter
4. 下記の内容をコピー＆ペースト
5. `Ctrl + S` で保存
...
初回起動時、「**Windowsファイアウォールがいくつかのアプリをブロックしています**」という通知が表示される場合があります。

**Docker Desktop Backend** のアクセス許可を求められたら、以下の設定で「**アクセスを許可する**」をクリック：

| ネットワーク | 設定 | 理由 |
|-------------|------|------|
| **プライベートネットワーク** | **チェックを入れる** | ローカル・家庭内で使用するため |
| **パブリックネットワーク** | チェック**しない** | 外部からのアクセスは不要 |

> **セキュリティ**: パブリックネットワークを許可すると、外部からアクセスされるリスクがあります。ローカル利用なら「プライベート」のみでOKです。
