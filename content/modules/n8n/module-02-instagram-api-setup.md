...
- [セクション1: Facebookページの作成](#セクション1-facebookページの作成)
- [セクション2: InstagramとFacebookページの連携](#セクション2-instagramとfacebookページの連携)
- [セクション3: Meta for Developers 登録](#セクション3-meta-for-developers-登録)
- [セクション4: アプリの作成](#セクション4-アプリの作成)
- [セクション5: Instagramテスターの追加](#セクション5-instagramテスターの追加)
- [セクション6: ユースケースのカスタマイズ](#セクション6-ユースケースのカスタマイズ)
- [セクション7: アクセストークンの取得](#セクション7-アクセストークンの取得)
- [セクション8: サーバーIP許可リストの設定（推奨）](#セクション8-サーバーip許可リストの設定推奨)
...
## セクション8: サーバーIP許可リストの設定（推奨）

n8nからInstagram APIを呼び出す際、Facebookが「不正なアクセス」と判断してブロックする場合があります。これを防ぐため、n8nサーバーのIPアドレスを許可リストに登録しておくことを推奨します。
...
3. 以下を設定：
   ...
   - **Method**: GET
4. 「Execute step」をクリック
5. 出力されたIPアドレスをメモ（例: `xxx.xxx.xxx.xxx`）

### Facebook開発者ダッシュボードで設定

![サーバーIP許可リスト設定画面](/n8n-setup/147-server-ip-allowlist.png)

1. [developers.facebook.com](https://developers.facebook.com/) にアクセス
2. 作成したアプリを選択
3. 左メニューの「アプリの設定」→「詳細設定」をクリック
4. 「セキュリティ」セクションを見つける
5. **「サーバーIP許可リスト」** にn8nサーバーのIPアドレスを入力
6. 「変更を保存」をクリック

### 注意事項

- 自宅回線など動的IPの場合、IPアドレスが変わると再設定が必要
- 複数のIPアドレスを登録可能（カンマ区切り）

### チェックポイント

- [ ] n8nでIPアドレスを確認した
- [ ] FacebookアプリのサーバーIP許可リストに登録した
